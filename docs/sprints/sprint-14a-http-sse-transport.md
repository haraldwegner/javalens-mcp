...
3. **Sandbox unblock (EXECSIM Session 2, 2026-05-17 — status TBD).** Antigravity's `nsjail` and Cursor's container model historically couldn't `fork+exec` arbitrary child processes — stdio MCP was fatally broken inside those sandboxes. Whether nsjail still blocks today is unverified; regardless, HTTP to a localhost URL works in sandboxes that allow loopback network access.
